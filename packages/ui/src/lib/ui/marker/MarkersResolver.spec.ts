...
describe('MarkersResolver', () => {
    ...
    it('Single marker is unchanged', () => {
        const point = new PositionedTimePoint(42, 'solo', 'solo', 2.5 * markerWidthPx)
        const resolvedMarkers = resolver.resolve([point]);
        expect(resolvedMarkers.length).toBe(1);
        expect(resolvedMarkers[0] instanceof Marker).toBeTrue();
        expect(resolvedMarkers[0].position).toBe(point.position);
    });

    it('Two markers are not grouped when they are farther than markerWidthPx', () => {
        const positions = [0, 1.01 * markerWidthPx];
        const resolvedMarkers = resolver.resolve(createPoints(positions));
    ...
    it('Markers not grouped when separated by slightly more than markerWidthPx', () => {
        const point1 = new PositionedTimePoint(1, 'a', 'a', 2 * markerWidthPx);
        const point2 = new PositionedTimePoint(2, 'b', 'b', 3.02 * markerWidthPx);
        const point3 = new PositionedTimePoint(3, 'c', 'c', 4.05 * markerWidthPx);
        const resolvedMarkers = resolver.resolve([point3, point1, point2]);
        expect(resolvedMarkers.length).toBe(3);
        expect(resolvedMarkers[0].position).toBe(point1.position);
        expect(resolvedMarkers[1].position).toBe(point2.position);
        expect(resolvedMarkers[2].position).toBe(point3.position);
    });

    it('Grouping 3 close markers and setting it with avg position', () => {
        const points = createPoints([0, 3, 4]);
        const resolvedMarkers = resolver.resolve(points);
        expect(resolvedMarkers.length).toBe(1);
        ...
        expect((groupingMarker as GroupingMarker).position).toBe(points[0].position);
        expect((groupingMarker as GroupingMarker).markers[0].position).toBe(points[0].position);
        expect((groupingMarker as GroupingMarker).markers[1].position).toBe(points[1].position);
        expect((groupingMarker as GroupingMarker).markers[2].position).toBe(points[2].position);
    });
    ...
    it('Grouping 2 close markers and space for another nested in 3 and another is alone setting it with avg position', () => {
        ...
        const points = createPoints(positions);
    ...
    it('Grouping many close markers that are close to the end', () => {
        ...
        const allGroupsFlat = [...group1, ...group2, ...group3, ...group4];
        const points = createPoints(allGroupsFlat);
        const resolvedMarkers: Array<Marker | GroupingMarker> = resolver.resolve(points);
...
function createPoints(positions: number[]): PositionedTimePoint[] {
    return positions.map((position, index) => {
        const value = 'not important - ' + index;
        const displayValue = 'not important either - ' + index;
        const time = position; // time is not relevant for the tests
        return new PositionedTimePoint(time, value, displayValue, position);
    });
}
