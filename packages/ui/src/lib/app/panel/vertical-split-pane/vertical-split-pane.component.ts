...
@Component({
    ...
    standalone: false,
    host: {
        '[style.--left-pane-width.px]': 'currentLeftPaneWidth'
    }
})
export class VerticalSplitPaneComponent {
    ...
    async ngAfterViewInit() {
        const defaultLeftPaneWidth = 62;
        this.currentLeftPaneWidth = await this.verticalSplitPaneLeftPaneWidthStorage.load() || defaultLeftPaneWidth;
        this.rightPaneWidthChange.next(this.rightPaneRef.nativeElement.getBoundingClientRect().width);
    }
